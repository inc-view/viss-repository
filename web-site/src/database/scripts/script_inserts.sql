...
INSERT INTO registro (registro, dtHora, fkHasComponente)
SELECT 
    ROUND(RAND() * 100, 0), -- Valor aleatório de PPM (de 0 a 100)
    DATEADD(HOUR, seq.seq, DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0)), -- Horário específico para cada hora
    5 -- ID de um componente PPM específico
FROM (
    SELECT 0 AS seq UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION 
    SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION 
...
SELECT 
    CASE 
        ...
        ELSE 40 -- Valor padrão para as horas restantes
    END,
    DATEADD(HOUR, seq.seq, CAST(CONVERT(VARCHAR(10), GETDATE(), 120) AS DATETIME)) AS dtHora, -- Horário específico para cada hora
    5 -- ID de um componente PPM específico
FROM (
    SELECT 0 AS seq UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION 
    SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION 
